        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_pos in self.legalPositions:
            ...
            for new_pos, prob in new_pos_dist.items():
                garbage[new_pos] += prob * self.beliefs[old_pos]
        #print "BELEIFS ARE" + beliefs.__str__()
        #print "GARBAGE ARE" + garbage.__str__()
        garbage.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(0, self.numParticles-1):
            self.parcles.append(self.legalPositions[i % len(self.legalPositions)])
    def observe(self, observation, gameState):
        """
        ...
        junk_stuff = []
        if noisyDistance == None:
            for j in range(0, self.numParticles-1):
                junk_stuff.append(self.getJailPosition())
            self.parcles = junk_stuff
            return
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        provided a helper method nameds self.getParticleWithGhostInJail(particle, ghostIndex)
